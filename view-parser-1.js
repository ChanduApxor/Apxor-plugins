...
function getMultiplier ()  {
  ...
  if (viewportMetaTag) {
    let content = viewportMetaTag.getAttribute("content")?.split(",") ?? [];
    let initial_scale = 1;
    content.forEach((currentValue, index, arr) => {
      if (currentValue.trimStart().indexOf("initial-scale") === 0) {
        initial_scale = parseFloat(currentValue.split("=")[1]);
        console.log("parseFloat", parseFloat(currentValue.split("=")[1]);
      }
    });
    console.log("initial_scale", initial_scale);
    return initial_scale;
  }
  return 1;
};
function getBounds(e, t, o, sf) {
  var n = e.getBoundingClientRect();
  return {
    left: Math.round(n.left * sf) + t,
    top: Math.round(n.top * sf) + o,
    right: Math.round(n.right * sf) + t,
    bottom: Math.round(n.bottom * sf) + o,
  };
}
...
function getViewHierarchy(e, t, o, q, b, n, r, a) {
  const sf = getMultiplier();
  var i = getBounds(e, t, o, sf),
    d = cssPath(e, !1),
    N = xPath(e, !1, r),
    fullCssPath = cssPath(e, !0),
    content = e && e.textContent ? e.textContent.trim() : "", // Get element's content (text)
    u = {
      id: d,
      bounds: i,
      view: e.tagName,
